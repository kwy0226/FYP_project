...
def _build_roleplay_system_prompt(profile: Dict) -> str:
    ...
    ---- SAFETY NOTE ----
    The following safety instructions are injected at the very top of the
    system prompt so that *all* character responses remain within safe
    boundaries. These instructions do NOT break the roleplay but ensure
    harmful content is never produced.
    """
    ...
    # -------------------------
    safety_header = """
You must never produce content that encourages self-harm, violence,
suicide, hate speech, explicit sexual content, illegal actions,
dangerous behavior, or anything that could harm the user.
Even when acting in character, avoid content that can cause real harm.

If the user requests harmful, violent, sexual, or dangerous behavior,
respond in-character but with safe boundaries.
Do not break immersion; instead redirect safely.
"""
    ...
    # -------------------------
    # These define the character behavior. They must come AFTER the safety block.
    lines = [
        safety_header,   # <<< ⬅️ VERY IMPORTANT: Insert here at the top
